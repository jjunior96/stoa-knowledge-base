...
Every deck here is listed in **[Stoa's catalogue](https://stoaflow.com.br/catalog)** —
search it, filter by subject, preview a deck and add it in one click.

<a href="https://stoaflow.com.br/catalog"><img src="./.github/assets/explore-the-catalogue.svg" alt="Explore the catalogue" height="36" /></a>

[Português](./docs/i18n/README.pt-BR.md) · [stoaflow.com.br/catalog](https://stoaflow.com.br/catalog)
...
Pick one and click — it opens in Stoa and shows you every card before asking
for anything.
...
| Deck | Cards | |
| --- | ---: | --- |
| [**CAP Theorem**](./subjects/system-design/)<br/>Trade-offs, linearizability e PACELC em sistemas distribuídos | 20 | <a href="https://stoaflow.com.br/import?path=subjects/system-design/decks/cap-theorem.csv"><img src="./.github/assets/add-to-stoa.svg" alt="Add to Stoa" height="28" /></a> |
...
3 of 14 decks. **[See all 14 in Stoa's catalogue →](https://stoaflow.com.br/catalog)** · [or by subject, below](#the-catalogue)
<!-- featured:end -->
...
Or they skip the link and browse everything at once, in Stoa itself:

```
stoaflow.com.br/catalog                          # every published deck
stoaflow.com.br/catalog?subject=system-design    # one subject
```
...
The catalogue lives in Stoa at
**[stoaflow.com.br/catalog](https://stoaflow.com.br/catalog)**. No account is
needed to browse it:

- **Search** by title or description, ignoring case and accents.
- **Filter by subject**. The address keeps the filter, so a filtered view can be
  shared as a link.
- **Preview** a deck, then **add it** to your account without leaving the page.
- Signed in, the decks already in your account are marked.

Here on GitHub, open a subject to see its decks, each with a button that adds
it to Stoa.

The buttons are plain links: GitHub's markdown sanitiser strips `target`, so
nothing in a README can force a new tab. Middle-click or ⌘/Ctrl-click if you
want to keep the catalogue open.

<!-- catalogue:start -->
<!-- Generated by scripts/build-manifest.mjs. Do not edit by hand. -->

| Subject | | Decks | In Stoa |
| --- | --- | ---: | --- |
| [**Arquitetura de Software**](./subjects/software-architecture/) | Princípios estruturais por trás do código: como dividir sistemas em módulos, onde a complexidade deve morar e como avaliar o design antes de escrevê-lo | 1 | [Browse →](https://stoaflow.com.br/catalog?subject=software-architecture) |
| [**Design UX/UI**](./subjects/ux-ui-design/) | Os princípios por trás de uma interface clara, acessível e fácil de usar | 1 | [Browse →](https://stoaflow.com.br/catalog?subject=ux-ui-design) |
...
14 deck(s) across 6 subject(s). Open a subject to see its decks, or **[browse them all in Stoa →](https://stoaflow.com.br/catalog)**
<!-- catalogue:end -->
...
The same manifest is what [Stoa's catalogue](https://stoaflow.com.br/catalog)
lists: every deck in it appears there, with its subject, and the decks in
[`featured.json`](#featured-decks) come first. The API caches the manifest (five
minutes by default), so a change reaches the catalogue and the links within a
few minutes of merging.
...
featured.json             # optional: the decks that lead this README and Stoa's catalogue
scripts/
...
opens, and the deck goes live when it merges — on its own link and in
[Stoa's catalogue](https://stoaflow.com.br/catalog).
...
The table at the top of this README is generated like everything else, but
*which* decks it shows is an editorial call rather than something derivable —
so it is the one input that is chosen by hand, in `featured.json`:
...
Three paths, in the order they should appear. Titles, descriptions and card
counts still come from the manifest, so a featured row cannot disagree with the
subject page it links to.

A path that is not a published deck **fails the build** rather than rendering a
button that leads nowhere — the first thing a reader clicks is the last place to
discover a broken link. The file is optional: without it the three largest decks
stand in, so a fresh clone still renders something true.

**Stoa's catalogue reads the same file.** On
[stoaflow.com.br/catalog](https://stoaflow.com.br/catalog) every deck listed here
is marked as featured and shown first, in this order, before the rest sorted by
title. The README shows only the first three; the catalogue shows them all.
Without the file, the catalogue simply has no featured decks.
...
- On merge, the deck is live with no deploy: within a few minutes its link works
  and it shows up in [Stoa's catalogue](https://stoaflow.com.br/catalog).
