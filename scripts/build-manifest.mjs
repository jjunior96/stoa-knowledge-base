...
const catalogueIndex = (manifest, labels) => {
  const rows = Object.entries(manifest.subjects)
    .sort(([, a], [, b]) => a.title.localeCompare(b.title))
    .map(([slug, subject]) => {
      const description = subject.description ? escapeCell(subject.description) : '';
      return `| [**${escapeCell(subject.title)}**](./subjects/${slug}/) | ${description} | ${subject.deckCount} |`;
    });
...
const replaceCatalogue = (file, block) => {
  ...
  if (start === -1 || end === -1) {
    throw new Error(
      `"${file}" has no ${START} / ${END} markers, so the catalogue has nowhere to go.`,
    );
  }
...
function renderOutputs(manifest) {
  ...
  for (const [file, labels] of [
    [join(ROOT, 'README.md'), LABELS.en],
    [join(ROOT, 'docs', 'i18n', 'README.pt-BR.md'), LABELS['pt-BR']],
  ]) {
    outputs.set(file, replaceCatalogue(file, catalogueIndex(manifest, labels)));
  }
...
let outputs;
try {
  outputs = renderOutputs(manifest);
} catch (cause) {
